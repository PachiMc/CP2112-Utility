...
INT CP2112_Open(INT devNumber, HID_SMBUS_DEVICE *device)
{
    ...
    if (status != HID_SMBUS_SUCCESS)
    {
        DWORD numDevices = 0;
        HidSmbus_GetNumDevices(&numDevices, VID, PID);
        fprintf(stderr, "HidSmbus_Open failed: %d (dev=%d). NumDevices=%lu\n", status, devNumber, (unsigned long)numDevices);
        return FALSE;
    }
    ...
    if (status != HID_SMBUS_SUCCESS)
    {
        fprintf(stderr, "HidSmbus_IsOpened returned status: %d\n", status);
        return FALSE;
    }
    if (!opened)
    {
        fprintf(stderr, "Device not opened (opened flag=0)\n");
        return FALSE;
    }
    return TRUE;   // Успех!
}
